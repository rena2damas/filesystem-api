...
class TestFilesystemSvc:
    def test_valid_list_files_returns_valid_data(self, svc, fs):
    ...
    def test_list_files_on_missing_file_raises_exception(self, svc):
        with pytest.raises(FileNotFoundError) as ex:
            assert svc.list_files(path="/tmp/missing.txt")
    ...
    def test_list_files_on_restricted_path_raises_exception(self, svc, fs):
        ...
        with pytest.raises(PermissionError) as ex:
            assert svc.list_files(path="/tmp/root")
        assert "Permission denied" in str(ex.value)

    def test_valid_stats(self, svc, fs):
        fs.create_file("/tmp/file.txt")
    ...
    def test_stats_on_restricted_path_raises_exception(self, svc, fs):
        ...
        with pytest.raises(PermissionError) as ex:
            assert svc.stats(path="/tmp/root")
        assert "Permission denied" in str(ex.value)
    ...
    def test_save_file(self, svc, fs, mocker):
        ...
        assert not fs.exists("/tmp/file.txt")
    ...
    def test_exists_path(self, svc, fs):
        ...
        assert svc.exists_path(filepath)
        assert svc.exists_path(dirpath)
    ...
    def test_remove_path(self, svc, fs):
        ...
        assert not svc.exists_path(filepath)
        assert not svc.exists_path(dirpath)
    ...
    def test_move_path(self, svc, fs):
        ...
        assert svc.exists_path(os.path.join(dst, os.path.basename(filepath)))
        assert svc.exists_path(os.path.join(dst, os.path.basename(dirpath)))
    ...
    def test_rename_path(self, svc, fs):
        ...
        assert svc.exists_path("/tmp/from.txt")
        assert svc.exists_path("/tmp/to")
    ...
    def test_isfile(self, svc, fs):
        ...
        assert not svc.isfile(dirpath)
        assert not svc.isfile("/tmp/missing.txt")
